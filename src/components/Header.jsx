...
const Header = () => {
  ...
  return (
    <nav className={`
      ...
      ${isScrolled 
        ? 'py-4 bg-white/80 backdrop-blur-xl border-slate-200 shadow-xl shadow-slate-200/50' 
    ...
    `}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <span className="text-2xl font-black text-slate-900 tracking-tighter">DUXX</span>
        </Link>
          ...
          {navItems.map((item) => (
            <Link 
              ...
              className={`text-sm font-semibold transition-all transform hover:-translate-y-0.5 ${
                location.pathname === item.path ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
              {item.name}
            </Link>
