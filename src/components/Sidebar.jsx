...
/**
 * [Component] LNB 사이드바 / 모바일 바텀시트 (카테고리 분리 v44.1)
 * @version 44.1
 * @description 
 * - 기능을 [바블/지하철/스타벅스] 세 가지 카테고리로 엄격하게 분리
 * - 각 카테고리 간 코드 간섭 원천 차단
 */
const Sidebar = ({ 
  ...
  // --------------------------------------------------------------------------
  // [공통 인프라] - 모든 카테고리가 공유하는 프레임워크 (수정 금지 영역)
  // --------------------------------------------------------------------------
  useEffect(() => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          ...
          {!memo.popped_at && (
            <button onClick={(e) => { e.stopPropagation(); onPop(memo.id, e); }} className="p-2.5 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 transition-all shadow-sm active:scale-90">
  ...
  // --------------------------------------------------------------------------
  // [카테고리 2: 지하철] - 지하철 마커 클릭 시 나오는 모든 것
  // --------------------------------------------------------------------------
  const SubwaySection = () => {
    if (!subwayArrivals) return null;
    if (subwayArrivals.loading) return <div className="p-8 flex flex-col items-center justify-center animate-pulse"><div className="w-12 h-12 bg-gray-100 rounded-full mb-4" /><p className="text-gray-400 font-bold text-sm">정보를 가져오는 중...</p></div>;

    const TrainItem = ({ train }) => (
      <div className={`p-4 rounded-2xl border ${train.arrivalType === '다음열차' ? 'bg-gray-50/50 border-gray-100' : 'bg-blue-50/50 border-blue-100'} transition-all`}>
        <div className="flex justify-between items-start mb-2">
          <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${train.arrivalType === '다음열차' ? 'bg-gray-200 text-gray-500' : 'bg-blue-500 text-white animate-pulse'}`}>{train.arrivalType}</span>
          <span className="text-[11px] font-bold text-gray-400">{train.time ? '예측 정보 있음' : '진입 중'}</span>
        </div>
        <div className="flex justify-between items-end">
          <div><p className="text-[15px] font-black text-gray-800">{train.dest} {train.direction}</p><p className="text-[12px] font-bold text-blue-600">{train.status}</p></div>
          <div className="text-right"><span className="text-[18px] font-black text-gray-900 tracking-tighter">{train.time}</span></div>
        </div>
      </div>
    );
  ...
  // --------------------------------------------------------------------------
  // [카테고리 3: 스타벅스] - 스타벅스 마커 클릭 시 나오는 모든 것
  // --------------------------------------------------------------------------
  const StarbucksSection = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 bg-[#00704a] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-green-700/20"><Coffee size={24} strokeWidth={2.5} /></div>
        ...
        <div className="p-5 bg-green-50/50 border border-green-100 rounded-[24px] space-y-4">
          <div className="flex items-start gap-3"><MapPin size={18} className="text-green-700 mt-0.5" /><p className="text-gray-700 font-medium text-[14px]">{starbucks.address}</p></div>
          <button className="w-full py-4 bg-white border border-green-100 rounded-2xl text-green-700 font-black text-[14px] flex items-center justify-center gap-2 active:scale-98 transition-all">길찾기 <ChevronRight size={16} strokeWidth={3} /></button>
        </div>
      </div>
    );
  };
  ...
  return (
    <AnimatePresence>
      {(memo || subwayArrivals || starbucks) && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] md:hidden cursor-pointer bg-transparent" onClick={onClose} />
          <motion.div style={{ height: isMobile ? sheetHeight : '100vh' }} initial={isMobile ? { height: 0 } : { x: -400, opacity: 0 }} animate={isMobile ? {} : { x: 0, opacity: 1 }} exit={isMobile ? { height: 0 } : { x: -400, opacity: 0 }} transition={{ type: 'spring', damping: 30, stiffness: 400 }} className="fixed z-[10000] bg-white shadow-2xl flex flex-col md:left-0 md:top-0 md:h-screen md:w-[380px] bottom-0 left-0 w-full rounded-t-[32px] md:rounded-none overflow-hidden">
            <motion.div className={`flex-shrink-0 touch-none select-none relative z-30 bg-white ${isMobile ? 'cursor-grab active:cursor-grabbing' : ''}`} onPan={isMobile ? handlePan : undefined} onPanEnd={isMobile ? handlePanEnd : undefined}>
              <div className="px-6 pt-5 pb-4">{isMobile && <div className="w-full flex justify-center pb-6"><div className="w-14 h-1.5 bg-gray-100 rounded-full" /></div>}{!isMobile && <div className="flex items-center justify-between mb-2"><span className="logo-font text-[20px] font-black text-[#FF4D00]">BABBLE</span><button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-all"><X size={20} className="text-gray-400" /></button></div>}</div>
            </motion.div>
