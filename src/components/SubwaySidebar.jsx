...
const SubwaySidebar = ({ 
  ...
  const handlePanEnd = (e, info) => {
    ...
    if (currentH > windowHeight * 0.65) {
      if (velocity > 400) animate(sheetHeight, windowHeight * 0.45, snapTransition);
      else animate(sheetHeight, windowHeight * 0.9, snapTransition);
    } else if (currentH > windowHeight * 0.2) {
      if (velocity < -400) animate(sheetHeight, windowHeight * 0.9, snapTransition);
      else if (velocity > 400) animate(sheetHeight, 0, snapTransition).then(() => onClose());
      else animate(sheetHeight, windowHeight * 0.45, snapTransition);
    } else {
